...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return await authMiddleware(req, async (_user: any) => {
    return await clientController.getClientById(req, resolvedParams.id);
  });
}
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return await authMiddleware(req, async (user: any) => {
    return await clientController.updateClient(req, resolvedParams.id, user);
  });
}
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = await params;
  return await authMiddleware(req, async (user: any) => {
    return await clientController.deleteClient(req, resolvedParams.id, user);
  });
}
